...
public class APIClient {
    ...
    public Response sendRequest(
        ...
        if (retryCount > 1) throw new RuntimeException(
            "sendRequest: Warning retryCount is: " + retryCount
        );
        if (ActivityUTILS.isMainThread()) {
            throw new NetworkOnMainThreadException();
        }
        ...
        Request request = locRequest
            .newBuilder()
            .header("X-Litecoin-Testnet", isTestNet ? "true" : "false")
        ...
        try {
            ...
            if (response.isRedirect()) {
                ...
                if (newUri == null) {
                    ...
                    !newUri.getHost().equalsIgnoreCase(BreadApp.HOST) ||
                    !newUri.getScheme().equalsIgnoreCase(PROTO)
                ) {
                    Timber.d(
                        "timber: sendRequest: WARNING: redirect is NOT safe: %s",
                        newLocation
                    );
                } else {
                    Timber.d(
                        "timber: redirecting: %s >>> %s",
                        request.url(),
                        newLocation
                    );
                    response.close();
                    return sendRequest(
                        new Request.Builder().url(newLocation).get().build(),
                        needsAuth,
                        0
                    );
                }
        ...
        if (
            ...
            try {
                if (response.code() != 200) {
                    Timber.d(
                        "timber: sendRequest: (%s)%s, code (%d), mess (%s), body (%s)",
                        request.method(),
                        request.url(),
                        response.code(),
                        response.message(),
