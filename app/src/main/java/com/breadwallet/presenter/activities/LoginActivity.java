...
public class LoginActivity extends BRActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        keyboard.setCustomButtonBackgroundColor(
            10,
                ...
                if (
                    ...
                    if (
                        ...
                        BRDialog.showCustomDialog(
                            app,
                            getString(
                                R.string.Send_cameraUnavailabeTitle_android
                            ),
                            getString(
                                R.string.Send_cameraUnavailabeMessage_android
                            ),
        ...
        new Handler()
            .postDelayed(
                () -> {
                    if (
                        fingerPrint != null && useFingerprint
    ...
    private void setCurrentLtcPrice() {
        ...
        CurrencyEntity currency = CurrencyDataSource.getInstance(
            this
        ).getCurrencyByIso(iso);
        if (currency != null) {
            final BigDecimal roundedPriceAmount = new BigDecimal(currency.rate)
                .multiply(new BigDecimal(100))
                .divide(new BigDecimal(100), 2, BRConstants.ROUNDING_MODE);
            formattedCurrency = BRCurrency.getFormattedCurrencyString(
                this,
                iso,
                roundedPriceAmount
            );
    ...
    private void handleDigitClick(Integer dig) {
        if (pin.length() < pinLimit) pin.append(dig);
        updateDots();
    }
    ...
    private void updateDots() {
        AuthManager.getInstance()
            .updateDots(
                ...
                dot5,
                dot6,
                R.drawable.ic_pin_dot_white,
                () -> {
                    inputAllowed = false;
                    if (
                        AuthManager.getInstance()
                            .checkAuth(pin.toString(), LoginActivity.this)
                    ) {
                        AuthManager.getInstance()
                            .authSuccess(LoginActivity.this);
                        unlockWallet();
                    } else {
        ...
        switch (requestCode) {
            case BRConstants.CAMERA_REQUEST_ID: {
                ...
                if (
                    ...
                    Timber.i(
                        "timber: onRequestPermissionsResult: permission isn't granted for: %s",
                        requestCode
                    );
