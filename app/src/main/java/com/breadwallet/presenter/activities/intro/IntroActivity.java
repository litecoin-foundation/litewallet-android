...
public class IntroActivity extends BRActivity implements Serializable {
    ...
    public IntroLanguageResource introLanguageResource =
        new IntroLanguageResource();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        countryLanguageAdapter = new CountryLanguageAdapter(
            this,
            introLanguageResource.loadResources()
        );
        listLangRecyclerView.setAdapter(countryLanguageAdapter);

        Language currentLanguage = LocaleHelper.Companion.getInstance()
            .getCurrentLocale();
        int currentIndex = introLanguageResource.findLanguageIndex(
            currentLanguage
        );
        countryLanguageAdapter.updateCenterPosition(currentIndex);
        new Handler()
            .post(() -> listLangRecyclerView.scrollToPosition(currentIndex));
        listLangRecyclerView.addOnScrollListener(
                ...
                public void onScrollStateChanged(
                    ...
                    if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                        ...
                        if (centerPosition != RecyclerView.NO_POSITION) {
                            countryLanguageAdapter.updateCenterPosition(
                                centerPosition
                            );
                            description.setText(
                                countryLanguageAdapter.selectedDesc()
                            );
                            showDialogForItem(
                                countryLanguageAdapter.selectedMessage()
                            );
                            listLangRecyclerView.smoothScrollToPosition(
                                centerPosition
                            );
                        }
                    }
        ...
        if (BRKeyStore.AUTH_DURATION_SEC != 300) {
            RuntimeException ex = new RuntimeException(
                "onCreate: AUTH_DURATION_SEC should be 300"
            );
            Timber.e(ex);
            throw ex;
        }
        ...
        if (masterPubKey != null && masterPubKey.length != 0) {
            ...
            isFirstAddressCorrect = SmartValidator.checkFirstAddress(
                this,
                masterPubKey
            );
        }
    ...
    private void showDialogForItem(String title) {
        ...
        Objects.requireNonNull(
            dialog.getWindow()
        ).setBackgroundDrawableResource(R.drawable.rounded_pop_up_intro);
                ...
                public void onClick(View v) {
                    if (
                        LocaleHelper.Companion.getInstance()
                            .setLocaleIfNeeded(
                                countryLanguageAdapter.selectedLang()
                            )
                    ) {
                        dialog.dismiss();
                        recreate();
            ...
            .execute(
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        Timber.d(
                            "timber: updateBundle DONE in %sms",
                            endTime - startTime
                        );
                    }
                }
