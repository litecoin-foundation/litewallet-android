...
public class BRSender {
    ...
    private static final long FEE_EXPIRATION_MILLIS = 72 * 60 * 60 * 1000L;
                ...
                new Runnable() {
                    ...
                    public void run() {
                        try {
                            if (sending) {
                                Timber.e(
                                    new NullPointerException(
                                        "sendTransaction returned because already sending.."
                                    )
                                );
                                return;
                            }
                            ...
                            if (
                                now - BRSharedPrefs.getFeeTime(app) >=
                                FEE_EXPIRATION_MILLIS
                            ) {
                                new Thread(
                                    new Runnable() {
                                        @Override
                                ...
                                if (
                                    time <= 0 ||
                                    now - time >= FEE_EXPIRATION_MILLIS
                                ) {
                                    Timber.d(
                            ...
                            if (!timedOut) tryPay(app, transactionItem);
                            else Timber.e(
                                new NullPointerException(
                                    "did not send, timedOut!"
                                )
                            );
                            return; //return so no error is shown
                        } catch (InsufficientFundsException ignored) {
                            errTitle[0] = app.getString(
                                R.string.Alerts_sendFailure
                            );
                        } catch (AmountSmallerThanMinException e) {
                            long minAmount = BRWalletManager.getInstance()
                                .getMinOutputAmountRequested();
                            errTitle[0] = app.getString(
                                R.string.Alerts_sendFailure
                            );
                            errMessage[0] = String.format(
                                Locale.getDefault(),
                                app.getString(
                                    R.string.PaymentProtocol_Errors_smallPayment
                                ),
                                BRConstants.litecoinLowercase +
                                new BigDecimal(minAmount).divide(
                                    new BigDecimal(100),
                        ...
                        if (
                            errTitle[0] != null && errMessage[0] != null
                        ) BRExecutor.getInstance()
                            .forMainThreadTasks()
                            .execute(
                                new Runnable() {
    ...
    private void tryPay(
        ...
        throws InsufficientFundsException, AmountSmallerThanMinException, SpendingNotAllowed, FeeNeedsAdjust {
        if (transactionItem == null || transactionItem.sendAddress == null) {
            Timber.d("timber: handlePay: WRONG PARAMS");
            String message = transactionItem == null
                ? "transactionItem is null"
                : "addresses is null";
            RuntimeException ex = new RuntimeException(
                "transactionItem is malformed: " + message
            );
            Timber.e(ex);
            throw ex;
        }
        ...
        long minOutputAmount = BRWalletManager.getInstance()
            .getMinOutputAmount();
        final long maxOutputAmount = BRWalletManager.getInstance()
            .getMaxOutputAmount();
        ...
        if (notEnoughForFee(transactionItem)) {
            ...
            if (maxOutputAmount == -1) {
                RuntimeException ex = new RuntimeException(
                    "getMaxOutputAmount is -1, meaning _wallet is NULL"
                );
                Timber.e(ex);
                throw ex;
            }
    ...
    private void confirmPay(
        ...
        if (transactionItem.isAmountRequested) {
            minOutput = BRWalletManager.getInstance()
                .getMinOutputAmountRequested();
        } else {
        ...
        if (transactionItem.sendAmount < minOutput) {
            final String bitcoinMinMessage = String.format(
                Locale.getDefault(),
        ...
        Timber.d(
            "timber: confirmPay: totalSent: %s, request.amount: %s, total limit: %s, limit: %s",
            BRWalletManager.getInstance().getTotalSent(),
            transactionItem.sendAmount,
            AuthManager.getInstance().getTotalLimit(ctx),
            BRKeyStore.getSpendLimit(ctx)
        );
    ...
    private String createConfirmation(
        Context ctx,
        ...
        long feesForTx = m.feeForTransaction(
            transactionItem.sendAddress,
            transactionItem.sendAmount
        );
        ...
        String formattedAmount = BRCurrency.getFormattedCurrencyString(
            ctx,
            iso,
        ...
        String formattedFees = BRCurrency.getFormattedCurrencyString(
            ctx,
            iso,
            BRExchange.getAmountFromLitoshis(
                ctx,
        ...
        String formattedTotal = BRCurrency.getFormattedCurrencyString(
            ctx,
            iso,
            BRExchange.getAmountFromLitoshis(ctx, iso, new BigDecimal(total))
        );
        ...
        return (
            receiver +
            "\n\n" +
            ctx.getString(R.string.Confirmation_amountLabel) +
            " " +
            formattedAmountLTC +
            " (" +
            formattedAmount +
            ")" +
            "\n" +
            ctx.getString(R.string.ConfirmationAllFees_Label) +
            " " +
            formattedFeesLTC +
            " (" +
            formattedFees +
            ")" +
            "\n" +
            ctx.getString(R.string.Confirmation_totalLabel) +
            " " +
            formattedTotalLTC +
            " (" +
            formattedTotal +
    ...
    String getReceiver(TransactionItem item) {
        boolean certified =
            item.certifiedName != null && item.certifiedName.length() != 0;
        return certified
            ? "certified: " + item.certifiedName
            : item.sendAddress;
    }
    ...
    private boolean isLargerThanBalance(
        Context app,
        TransactionItem transactionItem
    ) {
        return (
            transactionItem.sendAmount > 0 &&
            transactionItem.sendAmount >
            BRWalletManager.getInstance().getBalance(app)
        );
    }
    ...
    private boolean notEnoughForFee(TransactionItem transactionItem) {
        ...
        if (feeForTx == 0) {
            feeForTx = m.feeForTransaction(
                transactionItem.sendAddress,
                m.getMaxOutputAmount()
            );
            return feeForTx != 0;
        }
        return false;
